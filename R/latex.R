...
#' @param output_dir The output directory of the PDF file.
...
latexmk = function(
  ...
  pdf_file = gsub('tex$', 'pdf', file), output_dir = dirname(file), clean = TRUE
) {
  if (!grepl('[.]tex$', file))
  ...
  m1 = missing(pdf_file); m2 = missing(output_dir)
  if (!m1 && !m2 && dirname(pdf_file) != '.') stop(
    "When both 'output_dir' and 'pdf_file' are provided, the path 'pdf_file' ",
    "should not contain a directory name (you provided '", pdf_file, "')."
  )
  if (m2) output_dir = if (m1) {
    getOption('tinytex.latexmk.output_dir', dirname(file))
  } else dirname(pdf_file)
  # this will be the path of the original PDF generated by the LaTeX engine
  pdf = file.path(output_dir, gsub('[.]tex$', '.pdf', basename(file)))
  if (m1) pdf_file = pdf
  check_pdf = function() {
  ...
  if (emulation) {
    latexmk_emu(
      file, engine, bib_engine, c(engine_args, output_dir_arg), max_times,
      install_packages, output_dir, clean
    )
    return(check_pdf())
  }
  system2_quiet('latexmk', c(
    '-pdf -latexoption=-halt-on-error -interaction=batchmode', output_dir_arg,
  ...
  if (clean) system2('latexmk', c('-c', output_dir_arg), stdout = FALSE)
  check_pdf()
}
...
latexmk_emu = function(
  ...
  install_packages = FALSE, output_dir = '.', clean
) {
  ...
  aux_files = file.path(output_dir, paste(base, aux, sep = '.'))
